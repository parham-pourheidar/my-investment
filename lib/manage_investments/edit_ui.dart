...
class EditPage extends StatelessWidget {
  ...
  final String name;
  final String amount;
  EditPage(this.category, this.name, this.amount, {super.key});
  ...
  void createInvestmentFunc(
      String controlName, String amount, BuildContext context, String category) async {
    SharedPreferences instance = await SharedPreferences.getInstance();
    bool isStored = instance.getStringList(controlName) == null ? false : true;

    if (name == '') {
      snackBar(context, 'نام سرمایه را وارد کنید');
    } else if (amount == '') {
      snackBar(context, 'میزان سرمایه را وارد کنید');
    } else if (isStored && name != nameController.text) {
      snackBar(context, 'این نام قبلا استفاده شده است');
    } else {
      try {
        double.parse(amount);
        await instance.remove(name);
        await instance.setStringList(nameController.text, [amount, category]);
